...
void iLQR::generate_trajectory() {
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    // start).count();
    // cout << "compute_derivatives took: " << elapsed/1000. << " seconds." <<
    // endl;
    ...
    if (backPassDone)  //  serial backtracking line-search
    {
      for (int i = 0; i < Alpha.size(); i++) {
        ...
        if (expected > 0) {
          ...
          z = sgn(dcost);
#ifdef VERBOSE
          cout << "Warning: non-positive expected reduction: should not occur"
               << endl;
#endif
        }
...
// STEP 4: accept step (or not), print status
    ...
    if (iter == 0)
      std::cout
          << "iteration\tcost\t\treduction\texpect\t\tgrad\t\tlog10(lambda)\n";
#endif

    if (fwdPassDone) {
#ifdef SHOWPROGRESS
      printf("%-12d\t%-12.3g\t%-12.3g\t%-12.3g\t%-12.3g\t%-12.1f\n", iter,
             new_cost, dcost, expected, gnorm, log10(lambda));
#endif
    ...
    } else  // no cost improvement
    {
      ...
      printf("%-12d\t%-12s\t%-12.3g\t%-12.3g\t%-12.3g\t%-12.1f\n", iter,
             "NO STEP", dcost, expected, gnorm, log10(lambda));
#endif
...
int iLQR::backward_pass() {
  ...
  for (int i = (T - 1); i >= 0; i--)  // back up from end of trajectory
  {
    ...
    MatrixXd Qux_reg =
        cxu[i].transpose() + (fu[i].transpose() * Vxx_reg * fx[i]);
    MatrixXd QuuF = cuu[i] + (fu[i].transpose() * Vxx_reg * fu[i]) +
                    (lambda * MatrixXd::Identity(2, 2));
    ...
    if (v_free.any()) {
      MatrixXd Lfree;
      Lfree = -R.inverse() *
              (R.transpose().inverse() * rows_w_ind(Qux_reg, v_free));
    ...
    Vx[i] = Qx + K_i.transpose() * Quu * k_i + K_i.transpose() * Qu +
            Qux.transpose() * k_i;
    Vxx[i] = Qxx + K_i.transpose() * Quu * K_i + K_i.transpose() * Qux +
             Qux.transpose() * K_i;
...
VecOfVecXd iLQR::add_bias_to_u(const VecOfVecXd &u, const VecOfVecXd &l,
                               const double alpha) {
  VecOfVecXd new_u = u;
...
// Replaces this line from matlab: g_norm = mean(max(abs(l) ./ (abs(u)+1), [],
// 1));
