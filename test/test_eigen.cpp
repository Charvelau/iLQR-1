...
MatXd finite_differences(VecXd &x, VecXd (*foo)(const VecXd &)) {
  double eps = 0.0001;
  int n = x.size();
  MatXd J(n, n);
...
int main() {
  ...
  MatXd Lfree(m, n);
  MatXd R(2, 2);
  R << 1.0012, -0.0069, 0, 1.5170;
  MatXd Qux_reg(m, n);
  Qux_reg << -0.0124, 0.0101, 0.0113, 0.0100, -0.0196, 0.0065, -0.5332, 0.1886,
      -0.7361, 1.7398, -2.3677, -0.4859;
  VecXd free_v(2);
  free_v << 1, 1;
  ...
  // std::cout << "-------\n";
  //
  // Lfree = -R.inverse() * (R.transpose().inverse()*rows_w_ind(Qux_reg,
  // free_v));
