...
TEST(FiniteDiffTest, Vec2Scalar) {
  std::function<double(VectorXd)> quad_vec = [](VectorXd v) {
    return v.array().square().sum();
  };
...
TEST(FiniteDiffTest, Vec2Vec) {
  std::function<VectorXd(VectorXd)> add_ones = [](VectorXd v) {
    return v + VectorXd::Ones(v.size());
  };
...
TEST(FiniteDiffTest, LargeProblem) {
  std::function<VectorXd(VectorXd)> f = [](VectorXd v) {
    return (v.array() * 3).matrix();
  };
  VectorXd vec(1000);
  vec.setOnes();
