...
inline MatrixXd extract_bool_rowsandcols(const MatrixXd &mat,
                                         const VectorXd &bool_vec) {
...
inline void print_eigen(const std::string name,
                        const Eigen::Ref<const Eigen::MatrixXd> &mat) {
  if (mat.cols() == 1) {
    std::cout << name << ": " << mat.transpose().format(CleanFmt) << ";"
              << std::endl;
  } else {
